document.addEventListener("DOMContentLoaded", function() {
    ...
    // Vuốt Slide 
    var slide_mouseup = document.querySelector('.our-blog .box-body-slide');
    var countItems = document.querySelectorAll('.our-blog .box-footer ul li');
    slide_mouseup.addEventListener('mouseup', function() {
        var index;
        var slides = document.querySelector('.our-blog .box-body-slide ul');
        // lấy vị trí slide đang được active;
        var phantuhientai = document.querySelector('.our-blog .box-footer ul li.list-active');
        for (index = 0; phantuhientai = phantuhientai.previousElementSibling; index++) {}
        if (index < 0) {
            index = 0;
        } else if (index >= countItems.length - 1) {
            index = 0;
        } else {
            index++;
        }
